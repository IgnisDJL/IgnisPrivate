Namespace UI
    ...
    Public Class DataFilesAnalysisStepView
        ...
        Private labelProgressBar As Label
        ...
        Protected Overrides Sub initializeComponents()
            ...
            Me.labelProgressBar = New Label
            Me.labelProgressBar.TextAlign = ContentAlignment.MiddleLeft
            Me.labelProgressBar.AutoSize = False
            Me.Text = "Données analisée"
        ...
        Protected Overloads Overrides Sub ajustLayout()
            ...
            If (Not IsNothing(Me.labelProgressBar)) Then
                Me.labelProgressBar.Location = layout.FirstLabel_Location
                Me.labelProgressBar.Size = layout.FirstLabel_Size
                Me.progressBar.Location = layout.FirstProgressBar_Location
                Me.progressBar.Size = layout.FirstProgressBar_Size
            End If
        ...
        Private Sub newFileAnalysisStartedEventsHandler(productionDay As ProductionDay_1)

            'Console.WriteLine("New file being analysed : " & File.getFileInfo.Name & File.Date_.ToString(" (dd-MMM-yyyy)"))
        ...
        Private Sub progressEventHandler(current As Integer, total As Integer)

            Dim value As Double = 1 * (100 / 2) + (current / total) * (100 / 2)
            If (CInt(value) > Me.progressBar.Value) Then
                Me.Invoke(Sub() setProgressBarValue(value))
            End If
        End Sub
        ...
        Private Sub reset()
            ...
            Me.Controls.Remove(Me.labelProgressBar)
            Me.Controls.Remove(Me.progressBar)
            ...
            Me.labelProgressBar = Nothing
            Me.progressBar = Nothing
            'Me.label2 = Nothing
            'Me.label3 = Nothing
            ...
            Me.labelProgressBar = New Label
            labelProgressBar.Text = "Données analisée"
            Me.progressBar = New ProgressBar
            Me.progressBar.Style = ProgressBarStyle.Continuous
            nbProgressBars = 1

            Me.Controls.Add(labelProgressBar)
        ...
        Private ReadOnly Property MinProgressBarValue As Integer
            Get
                ...
                If (Not IsNothing(Me.progressBar)) Then
                    minVal = Math.Min(minVal, Me.progressBar.Value)
                End If
