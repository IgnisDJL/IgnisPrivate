Public Class HybridStatistics
    ...
    Public Overrides Sub compute(cycles As List(Of Cycle), ByRef events As EventsCollection)
        ...
        For Each currentCycle As Cycle In cycles
            ...
            If (TypeOf currentCycle Is CSVCycle) Then
                ...
                ' #remove - when we have confirmation that they shouldn't be analysed (if not dont forget to put time diff back (look at csvstats))
        ...
        ' #refactor
        computeStartTime(firstCSVCycle, firstLOGCycle, If(events.START_EVENTS.Count > 0, events.START_EVENTS.First, Nothing))
        computeEndTime(lastCSVCycle, lastLOGCycle, events)

        Me.AllMixes.Sort()
        Me.AllAsphalts.Sort()

        computeOtherAndTotalMix(lastCSVCycle, lastLOGCycle)
    ...
    Private Sub computeEndTime(lastCSVCycle As CSVCycle, lastLOGCycle As LOGCycle, ByRef events As EventsCollection)

        Dim lastStopEvent As SingleEvent = If(events.STOP_EVENTS.Count = 0, Nothing, events.STOP_EVENTS.Last)
    ...
    Private Sub computeOtherAndTotalMix(lastCSVCycle As CSVCycle, lastLOGCycle As LOGCycle)
