...
''' <summary>
''' Generates the model for the report in the models directory.
''' </summary>
''' <remarks></remarks>
Public Class CSVModel
    ...
    ''' <summary>
    ''' Creates the excel file that will serve as model for the report that will contain the data from the csv files
    ''' </summary>
    ''' <remarks></remarks>
    Public Overrides Function generateModel() As Microsoft.Office.Interop.Excel.Workbook

        ' Delete the file if it already existed
        If (CSVModel.fileExists) Then
            ...
            Do
                ...
                Try
                    ...
                    System.IO.File.Delete(Constants.Paths.CSV_MODEL)
        ...
        Dim sheetSettings = XmlSettings.Settings.instance.Report.Excel.CSV_SHEET
        ...
        Me.saveAs(xlsWorkBook, Constants.Paths.CSV_MODEL, Constants.Output.Excel.REPORT_EXTENSION)
    ...
    Public Shared Function fileExists() As Boolean
        Return System.IO.File.Exists(Constants.Paths.CSV_MODEL)
    End Function
End Class
